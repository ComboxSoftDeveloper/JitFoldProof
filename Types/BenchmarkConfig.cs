...
/// <summary>
/// Три рантайма одним прогоном плюс дизассемблер.
/// Дизассемблер здесь не украшение: почти всё, что проверяет этот проект,
/// доказывается не наносекундами, а отсутствием инструкций в машинном коде.
/// </summary>
public sealed class BenchmarkConfig : ManualConfig
{
    ...
    public BenchmarkConfig(bool disassembly)
    {
        ...
        // по которой замер не состоялся. Журнал в файл тоже нужен: в консоли
        // остаётся не всё, а разбирать сбой больше нечем.
        AddLogger(ConsoleLogger.Default);

        // Без провайдера колонок сводная таблица выходит пустой: замеры считаются,
        // но показывать их нечем. ManualConfig не подставляет его сам.
